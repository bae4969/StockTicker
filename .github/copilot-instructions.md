...
- **import 순서**: 표준 라이브러리 → 외부 패키지 → 프로젝트 내부 (`from core import config`, `from core import util`, `doc.settings`)
...
# 실행 (doc/settings.py 설정 필요 — SQL_HOST, SQL_ID, SQL_PW, KI_API_KEY_LIST)
python main.py
...
### 주의사항
- `doc/settings.py`는 `.gitignore` 대상이므로 별도 생성 필요 (SQL 접속정보, API 키)
